...
public abstract class RelativeInstructionRelocationEmitter extends InstructionRelocationEmitter {
	public RelativeInstructionRelocationEmitter(Program program, RelocationTable relocationTable,
			Function function) {
	...
	public boolean emitRelocation(Instruction instruction, Reference reference,
		...
		relocationTable.addRelativePC(fromAddress.add(opValue.offset), opValue.length, symbol.name,
			symbol.offset - getInstructionAddend(instruction));
		return true;
	}
}
